...
writeLines(c("# RQ3 run report","",paste0("Upstream: ",RQ1_VERSION),"Sufficiency: S_k(epsilon) uses expected absolute standardized distortion A.","epsilon_entry(c)=A(c): the minimum tolerance at which an observed configuration becomes sufficient.","Duration references use the first seven valid dates when at least seven valid dates occur within the protocol interval.","Multidimensional results use facet-specific maximal supports; placement and optical states remain incomparable; Pareto dominance applies only to temporal resolution and duration."),file.path(OUT_RESULTS,"RQ3_RUN_REPORT.md"))
