# Fig. 2 only — context dependence of distortion.
...
predictor_overall <- coef_metric |>
  group_by(term) |>
  summarise(overall_abs = median(abs(estimate), na.rm = TRUE), n_display_units = n(), .groups = "drop")
...
context_task <- context_task |>
  ...
  select(dimension, comparison_pair_id, metric, outcome, external_beyond_state, state_beyond_external) |>
  pivot_longer(c(external_beyond_state, state_beyond_external), names_to = "information", values_to = "delta_r2") |>
  mutate(
    information = recode(information,
      external_beyond_state = "External beyond state", state_beyond_external = "State beyond external"),
    dimension = factor(dimension, levels = DIMENSIONS, labels = unname(DIM_TITLES[DIMENSIONS])),
  ...
  group_by(dimension, metric, outcome, outcome_label, information) |>
...
joint_cv_metric <- performance |>
  filter(str_detect(validation_scheme, "^participant_grouped"), model_family == "joint", is.finite(r2)) |>
...
joint_cv_summary_panel <- joint_cv_metric_panel |>
  ...
  summarise(
    ...
    r2_q75 = quantile(r2, .75, na.rm = TRUE, names = FALSE), .groups = "drop"
  ) |>
  mutate(
    class_num = as.integer(metric_class),
    class_offset = (class_num - (length(METRIC_CLASSES) + 1) / 2) * .045,
...
joint_cv_metric_panel <- joint_cv_metric_panel |> mutate(r2_plot = squish_to_limits(r2, joint_cv_window_panel))
...
p_frac_panel <- ggplot(joint_cv_positive_panel, aes(fraction_plot, y)) +
  geom_vline(xintercept = 0, linewidth = .27, color = "#A1A6A9") +
  geom_segment(aes(x = 0, xend = fraction_plot, y = y, yend = y, color = outcome_label),
               linewidth = 2.4, alpha = .82, lineend = "butt") +
  geom_text(aes(x = fraction_plot, label = label, hjust = if_else(fraction_plot >= 0, -.08, 1.08)),
...
right_top <- cowplot::plot_grid(
  metric_legend_right, p2b, ncol = 1, rel_heights = c(.10, .90),
  align = "v", axis = "l", greedy = TRUE
)
right_column <- cowplot::plot_grid(
  right_top, p2c, ncol = 1, rel_heights = c(.70, .30),
  align = "v", axis = "l", greedy = TRUE
)
p2 <- cowplot::plot_grid(
  p2a, right_column, ncol = 2, rel_widths = c(.64, .36),
...
message("Fig. 2 complete: contextual predictor atlas, conditional geometry, and held-out predictability.")
