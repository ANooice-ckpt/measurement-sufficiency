# Canonical RQ2 recovery: conventional calibration followed by residual learning.
...
recovery_signature <- function(unit, core_version, requested = NULL) {
  ...
  recovery_require(unit, c("core_artifact_version", "config_id", "analysis_unit_type",
    "placement", "optical", "resolution_s", "support_id", "site", "Id", "Date", hours),
    "Frozen low-configuration hourly basis")
...
recovery_resolve_parts <- function(upstream, repo_root = ".") {
  if (!dir.exists(upstream$part_dir)) upstream$part_dir <- file.path(repo_root, "results", "rq1", "pairwise_parts",
...
recovery_inputs <- function() {
  ...
  resolved <- recovery_resolve_parts(upstream); upstream <- resolved$upstream; parts <- resolved$paths
  problems <- c(problems, resolved$problems)
  summary <- recovery_read_csv(paths[["summary"]])
  tryCatch(rq1_assert_summary_version(upstream, summary), error = function(e) problems <<- c(problems, conditionMessage(e)))
  tryCatch(ms_assert_version(summary, "core_artifact_version", core), error = function(e) problems <<- c(problems, conditionMessage(e)))
  ...
  recovery_require(scales, c("comparison_lattice", "metric", "metric_geometry", "standardizer", "scale_anchor_config"), "Frozen scales")
  ...
  diary_paths <- setNames(vapply(sort(unique(context$site)), raw_data_path, character(1), modality = "lightexposurediary"),
    sort(unique(context$site)))
  ...
  if (!length(problems)) {
    ...
    temporal <- recovery_temporal_context(obj, core, version); temporal_sources <- obj$sources
  }
...
recovery_pairs <- function(inputs) {
  ...
  x <- left_join(x, inputs$scales,
    by = c("comparison_lattice", "metric", "metric_geometry", "scale_anchor_config"), relationship = "many-to-one")
  ...
  ok <- x$eligible
  if (any(!is.finite(x$delta[ok])) || any(abs(err[ok] - x$delta[ok]) > 1e-8 * (1 + abs(x$delta[ok]))))
    stop("Frozen delta disagrees with paired-value geometry")
  if (any(!is.finite(x$z[ok])) || any(abs(err[ok] / x$standardizer[ok] - x$z[ok]) > 1e-8 * (1 + abs(x$z[ok]))))
    stop("Frozen scale/orientation does not reproduce RQ1 z")
...
# -----------------------------------------------------------------------------
# Conventional calibration + residual learning
# -----------------------------------------------------------------------------
# Baseline calibration is intentionally low-capacity and deployable. For linear
# metrics it is an affine mapping Y_H = a + b Y_L. Circular metrics use the
# corresponding affine map in sin/cos coordinates. All auxiliary branches share
# this exact fitted baseline; flexible ML is reserved for the residual discrepancy.
recovery_calibration <- function(tr, te, circular) {
  if (!circular) {
    xtr <- cbind(intercept = 1, low = as.numeric(tr$candidate_value))
    xte <- cbind(intercept = 1, low = as.numeric(te$candidate_value))
    y <- as.numeric(tr$reference_value)
    beta <- tryCatch(qr.solve(xtr, y, tol = 1e-10), error = function(e) rep(NA_real_, 2L))
    if (any(!is.finite(beta))) beta <- c(mean(y - tr$candidate_value), 1)
    train_pred <- as.numeric(xtr %*% beta); test_pred <- as.numeric(xte %*% beta)
    model <- list(model_type = "affine_calibration", coefficients = beta,
      columns = colnames(xtr), target = "Y_H_from_Y_L")
    return(list(train_prediction = train_pred, prediction = test_pred,
      fallback = rep(FALSE, nrow(te)), model = model))
  }
  angle <- function(x) x * 2*pi/86400
  xmat <- function(x) cbind(intercept = 1, low_sin = sin(angle(x)), low_cos = cos(angle(x)))
  ...
  if (any(!is.finite(beta))) {
    d <- angle(recovery_delta(tr$reference_value, tr$candidate_value, TRUE))
    shift <- atan2(mean(sin(d)), mean(cos(d)))
    make_pred <- function(x) (x + shift * 86400/(2*pi)) %% 86400
    return(list(train_prediction = make_pred(tr$candidate_value), prediction = make_pred(te$candidate_value),
      fallback = rep(FALSE, nrow(te)), model = list(model_type = "circular_constant_offset",
        offset_seconds = shift * 86400/(2*pi), target = "Y_H_from_Y_L")))
  }
  decode <- function(v, fallback_value) {
    norm <- sqrt(rowSums(v^2)); bad <- !is.finite(norm) | norm < 1e-10
    out <- atan2(v[, 1], v[, 2]) * 86400/(2*pi); out <- out %% 86400
    out[bad] <- fallback_value[bad] %% 86400
  ...
  list(train_prediction = trd$value, prediction = ted$value, fallback = ted$fallback,
    model = list(model_type = "circular_affine_calibration", coefficients = beta,
      columns = colnames(xtr), target = "sin_cos_Y_H_from_sin_cos_Y_L"))
}

recovery_design <- function(tr, te, predictors, screen = TRUE) {
  allowed <- lapply(c("signature", "context_only", "context"), recovery_layer_predictors)
  if (!any(vapply(allowed, identical, logical(1), predictors)))
    stop("Residual predictors must match a prespecified auxiliary information state")
...
recovery_apply_residual <- function(baseline, pr, circular) {
  ...
  if (circular) {
    ...
    correction <- as.numeric(pr); prediction <- baseline + correction
  }
...
recovery_boost <- function(inner, inner_y, full, full_y, config, seed) {
  ...
  log <- attr(tuned, "evaluation_log"); if (is.null(log)) log <- tuned$evaluation_log
...
recovery_predict <- function(tr, te, predictors, circular, lambda, learner = "ridge",
  ...
  d <- recovery_design(tr, te, predictors, screen = learner == "ridge")
  y <- recovery_residual_target(tr$reference_value, calibration$train_prediction, circular)
  details <- list()
  if (learner == "ridge") {
    ...
    inner <- recovery_design(tr[!v, ], tr[v, ], predictors, screen = FALSE)
    ...
    details <- list(target_terms = colnames(y), boosters = lapply(fits, `[[`, "model"),
      inner_participants = split, inner_calibration = inner_cal$model,
      inner_centers = inner$centers, inner_scales = inner$scales,
  ...
  list(prediction = applied$prediction, correction = applied$correction, fallback = applied$fallback,
    model = c(list(learner = learner, target = "residual_after_conventional_calibration",
      baseline = calibration$model, columns = colnames(d$tr), centers = d$centers,
...
recovery_fit <- function(task) {
  ...
  for (f in sort(unique(x$fold))) {
    ...
    x$calibration[vi] <- cal$prediction; x$calibration_fallback[vi] <- cal$fallback
  ...
  pred <- bind_rows(lapply(recovery_states(), function(state) {
    ...
    x |> transmute(site, Id, Date, support_id, participant_key, fold, context_row_present,
      ...
      standardized_error = error / standardizer, circular_fallback = .env$fallback)
  }))
...
recovery_worker_exports <- function() c("recovery_task", "recovery_fit", "recovery_predict", "recovery_calibration",
  "recovery_design", "recovery_residual_target", "recovery_apply_residual", "recovery_delta", "recovery_predictors",
  "recovery_atomic", "rq2_model_helpers", "recovery_inner", "recovery_boost", "recovery_xgb_config",
...
recovery_smoke <- function(inputs) {
  ...
  seed <- as.integer(Sys.getenv("RQ2_RECOVERY_SEED", "20260912")); folds <- as.integer(Sys.getenv("RQ2_RECOVERY_FOLDS", "5"))
  ...
  root <- tempfile("recovery_real_smoke_"); dir.create(root); on.exit(unlink(root, recursive = TRUE), add = TRUE)
  tasks <- list()
  ...
  report <- bind_rows(lapply(refs, function(r) {
    ...
    for (model in obj$models) {
      if (identical(model$state, "calibration")) {
        ...
        allowed <- c("intercept", recovery_layer_predictors(model$state), paste0(recovery_layer_predictors(model$state), "_missing"))
    ...
    tibble(learner = tasks[[r$index]]$learner, metric = obj$meta$metric,
      ...
      heldout_days = unname(counts["raw"]), states = length(counts), fitted_models = length(obj$models))
  }))
  stopifnot(identical(cache_md5, tools::md5sum(cache_path))); print(report)
  message("PASS real smoke: ", nrow(report), " tasks; conventional calibration + nested residual learning; ",
...
recovery_run <- function(inputs) {
  ...
  integer_env <- function(name, default, minimum) {
    ...
    if (length(v) != 1L || !is.finite(v) || v < minimum) stop("Invalid ", name); v
  }
  ...
  provenance <- list(recovery_version = "rq2_recovery_v6_contextual_residual_calibration",
    rq1_analysis_version = inputs$version, core_artifact_version = inputs$core,
    analysis_design_id = ms_analysis_design_id(), input_md5 = tools::md5sum(inputs$paths), code_md5 = tools::md5sum(code),
    ...
    fitted_states = recovery_states(),
    calibration = "outer-training affine Y_H~Y_L; circular metrics use affine sin/cos mapping; no boosted calibration",
    residual_estimand = "predict discrepancy remaining after conventional calibration",
    factorial_context_state = "calibrated Y_L plus 18 daily + 32 daypart context predictors without low-measurement signature",
    ...
    inner_validation = "20% of outer-training participants; inner calibration refit on inner-training only; residual early stopping on inner-held-out participants; full-training refit",
    ...
    model = "Conventional calibration prior plus residual discrepancy learning; factorial S/C branches; primary XGBoost with nested early stopping; ridge sensitivity; no target-state predictors")
  run_id <- recovery_hash(provenance); out <- file.path("results/rq2/recovery", inputs$version, run_id)
  ...
  unavailable <- x |> filter(!eligible) |> count(dimension, comparison_pair_id, metric, support_id, pair_reason, available, name = "n_rows")
  ...
  tasks <- lapply(seq_len(nrow(catalog)), function(i) list(index = i,
    input = input_paths[[catalog$base_task_index[i]]], output = file.path(out, "checkpoints", sprintf("task_%04d.rds", i)),
    learner = catalog$learner[i], seed = seed + catalog$base_task_index[i], xgb_config = recovery_xgb_config(),
    run_id = run_id, lambda = lambda, meta = catalog[i, ]))
  rm(x, groups); invisible(gc(FALSE))
  message("Recovery: ", length(tasks), " tasks; ", workers, " workers; ", folds, " grouped folds; conventional calibration + residual learning; ", out)
  ...
  recovery_atomic(list(run_id = run_id, complete = !any(status$status == "failed"), statuses = status,
    heldout_errors = states, provenance = provenance), file.path(out, "recovery_manifest.rds"))
...
recovery_summaries <- function(states, out, floor) {
  ...
  bins <- comparison |> group_by(learner, comparison_pair_id, state, raw_magnitude_bin) |>
    summarise(n_metrics = n(), raw_min = min(A_raw), raw_max = max(A_raw),
      delta_median = median(delta_A), delta_q25 = quantile(delta_A, .25), delta_q75 = quantile(delta_A, .75), .groups = "drop")
...
if (sys.nframe() == 0L) {
  ...
  if (!command %in% allowed || length(args) != 1L)
    stop("Use --check-inputs, --build-context, --smoke-test, or --run")
  inputs <- recovery_inputs()
  if (length(inputs$problems)) stop(paste(c(inputs$problems, "No upstream process was invoked. Supply the existing frozen artifacts."), collapse = "\n"))
