...
if (sys.nframe() == 0L) {
  ...
  if (length(args) != 1L || !args %in% c("--check-inputs", "--build-context", "--smoke-test", "--run"))
  ...
  if (args == "--check-inputs") message("Frozen paths and context schemas validated; daypart cache ready. Pair contents are checked by --smoke-test/--run.")
  else if (args == "--build-context") message("Daypart context ready; reused=", inputs$temporal_reused)
  else if (args == "--smoke-test") recovery_smoke(inputs)
