...
interaction_scope<-tibble(dimension_pair=c("placement x optical","placement x temporal","optical x temporal","placement x duration","optical x duration","temporal x duration"),status=c("estimated","estimated","estimated","not population-estimated","not population-estimated","not population-estimated"),scope_reason=c(rep("observable joint core cells on the required common support",3),rep("duration-containing second-order contrasts are outside the primary cross-dimensional estimand; duration is evaluated with protocol-anchored windows and facet-specific sufficiency downstream",3)))
