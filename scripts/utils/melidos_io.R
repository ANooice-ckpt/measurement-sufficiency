...
sites_for_modality <- function(sites, modality) if (modality %in% c("light_chest", "light_wrist", "light_chest_1minute", "light_wrist_1minute")) setdiff(sites, "MPI") else sites
melidos_url <- function(site, modality) {
  folder <- if (grepl("^light_", modality)) "light" else "continuous"
  sprintf("https://raw.githubusercontent.com/MeLiDosProject/%s/main/data/imported/%s/%s.RData",
          unname(melidos_repositories[[site]]), folder, modality)
}
raw_data_path <- function(site, modality) file.path("data", "raw", "melidos", paste0(site, "__", modality, ".RData"))
object_name_for_modality <- function(modality) if (identical(modality, "sleepdiaries")) "sleepdiary" else sub("1minute$", "1min", modality)
load_raw_file <- function(path, modality) {
  ...
  if (!exists(object_name, envir = env, inherits = FALSE)) stop(sprintf("Expected '%s' in %s; found %s", object_name, path, paste(ls(env), collapse = ", ")))
  env[[object_name]]
}
