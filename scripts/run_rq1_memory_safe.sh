...
{
  echo "===== RQ1 RESUME: $(date --iso-8601=seconds) ====="
  ...
  Rscript --vanilla -e 'parse("scripts/10_rq1_analysis.R"); cat("Canonical RQ1 parses successfully\n")'
  Rscript scripts/10_rq1_analysis.R
  echo "===== RQ1 COMPLETE: $(date --iso-8601=seconds) ====="
} 2>&1 | tee -a "${LOG}"
