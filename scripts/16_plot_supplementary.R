...
# All supplementary figure construction is intentionally centralized here.
# Sourcing the main-figure entrypoints reconstructs their unchanged data
# objects before the FigS-only drawing blocks below are evaluated.
...
ms_plot_write_manifest(
  ...
  tibble(
    ...
    input_artifact = c('rq1_pairwise_change_long (derived Spearman) + rq1_pairwise_summary + rq1_local_transition_summary', 'rq1_pairwise_summary + rq1_metric_availability', 'rq1_pairwise_summary', 'rq1_metric_availability'),
...
ms_plot_save(p2_context_s, file.path(OUT_DIR, "FigS_RQ2_context_increment.png"), 7.6, 4.8)
  ...
  source("scripts/13b_plot_fig3.R", local = TRUE)
...
ms_plot_write_manifest(
  file.path(OUT_DIR, "figure_artifact_manifest.csv"),
  tibble(
    figure = c('Fig2_RQ2', 'Fig3_RQ2', 'FigS_RQ2_conditional_atlas', 'FigS_RQ2_state_spread', 'FigS_RQ2_context_increment', 'FigS_RQ2_gamma_atlas', 'FigS_RQ2_model_performance'),
    input_artifact = c('rq2_conditional_geometry+rq2_model_coefficients', 'rq2_gamma_summary', 'rq2_conditional_geometry', 'rq2_conditional_geometry', 'rq2_model_performance', 'rq2_gamma_summary', 'rq2_model_performance'),
    core_artifact_version = CORE_VERSION,
    rq1_analysis_version = RQ1_VERSION,
...
ms_plot_save(fig4s, file.path(OUT_DIR, "FigS_RQ3_single_dimension_detail.png"), 12.5, 8.0)
  ...
  source("scripts/15b_plot_fig5.R", local = TRUE)

ms_plot_write_manifest(
  file.path(OUT_DIR, "figure_artifact_manifest.csv"),
  tibble(
    figure = c('Fig4_RQ3', 'Fig5_RQ3', 'FigS_RQ3_single_dimension_detail'),
    input_artifact = c('rq3_observed_stability+sufficiency+unordered_substitutability', 'rq3_joint_summary+rq3_pareto_occupancy', 'rq3_convergence_profile+sufficiency'),
    core_artifact_version = CORE_VERSION,
    rq1_analysis_version = RQ1_VERSION,
    rq2_analysis_version = NA_character_,
